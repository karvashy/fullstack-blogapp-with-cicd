...
describe('UserAuth: testing blog functions from a logged in account', () => {
    beforeEach(async () => {
        ...
        await api.post('/api/blogs').set({ 'Authorization': 'Bearer '+response.body.token }).send({ ...selectedBlog, user: response.body.id })
    }, 20000)
    ...
    test('UserAuth: testing  blog deletion based on user token', async () => {
        ...
        const blogResponse = await api.get('/api/blogs')
        const selectedBlog = blogResponse.body[blogResponse.body.length - 1]
        console.log('token in blog deletion',token,'selectedBlog',selectedBlog)
        await api.delete(`/api/blogs/${selectedBlog.id}`)
            .set({ 'Authorization':'Bearer '+token })
            .expect(204)
    })
